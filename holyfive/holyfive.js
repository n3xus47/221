...
// Efekt mieszania liter (scramble effect)
document.addEventListener("DOMContentLoaded", function() {
    const letters = "ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789!@#$%^&*()_+-=[]{}|;:,.<>/?\"";
    const elements = document.querySelectorAll("[data-value]");

    elements.forEach(element => {
        // Zapisz oryginalną wartość z data-value
        const originalValue = element.dataset.value;
        // Zapisz uppercase wersję dla efektu
        ...
        element.addEventListener("mouseover", event => {  
            ...
            element.interval = setInterval(() => {
                event.target.innerText = uppercaseValue
                    .split("")
                    .map((letter, index) => {
                        if(index < iteration) {
                            return uppercaseValue[index];
                        }
                        
                        // Losowy znak z tablicy wielkich liter
                        return letters[Math.floor(Math.random() * letters.length)];
                    })
                    .join("");
                
                // Poprawione sprawdzanie zakończenia iteracji z dokładniejszym warunkiem
                if(iteration >= uppercaseValue.length){ 
                    clearInterval(element.interval);
                }
        ...
        // Po zakończeniu efektu przywróć wielkie litery
        element.addEventListener("mouseout", event => {
            if (element.interval) {
                clearInterval(element.interval);
                element.innerText = uppercaseValue;
            }
        });
    });
});